      ...
      <Card className={cn(acted && "opacity-60")}>
        <CardContent className="space-y-5 p-5 sm:p-6">
          ...
          <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="font-display text-lg font-semibold text-ink-50">{play.clientName}</h3>
              ...
              <p className="mt-1 text-sm text-ink-400">{record.trigger}</p>
            ...
            <div className="shrink-0 text-left sm:text-right">
              ...
              <p
                className={cn(
                  "stat-mono text-2xl font-semibold",
                  play.winnability >= 60
                    ? "text-optimal"
                    : play.winnability >= 40
                      ? "text-watch"
              ...
              <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                {play.achievements.map((a) => (
                  <div key={a.headline} className="hairline rounded-xl bg-ink-900/50 p-3">
                    <p className="text-sm font-medium text-ink-50">{a.headline}</p>
                    <Cite e={a.evidence} />
                  </div>
                ))}
              </div>
            ...
            <div className="mt-2 space-y-2">
              {play.likelyReasons.map((r) => (
                <div key={r.hypothesis} className="rounded-xl border border-watch/20 bg-watch/[0.04] p-3">
                  <p className="text-sm leading-relaxed text-ink-100">{r.hypothesis}</p>
                  {r.evidence.map((e) => (
          ...
          <section>
            ...
            <p className="mt-2 select-text rounded-xl border border-gold-400/20 bg-gold-400/[0.05] p-4 text-[15px] leading-relaxed text-ink-100">
              {play.openingLine}
            </p>
          ...
          <section>
            ...
            <div className="mt-2 hairline rounded-xl bg-ink-900/50 p-4">
              <p className="text-sm font-medium text-ink-50">{play.offer.title}</p>
              <p className="mt-1 text-[13px] leading-relaxed text-ink-400">{play.offer.detail}</p>
          ...
          <details className="group">
            <summary className="flex cursor-pointer list-none items-center gap-2 text-[12px] text-ink-500 hover:text-ink-300">
              <FileSearch className="h-3.5 w-3.5" />
              Everything this play read ({play.evidence.length} records)
            </summary>
            <div className="mt-2 space-y-1.5">
              {play.evidence.map((e) => (
                <div key={`${e.source}:${e.claim}`} className="flex flex-wrap gap-x-2 text-[12px]">
...
function Cite({ e }: { e: Evidence }) {
  return (
    <p className="mt-1.5 flex flex-wrap items-baseline gap-x-1.5 text-[11px] leading-relaxed text-ink-500">

...
function LineRow({ line }: { line: CostLine }) {
  ...
  return (
    <li className="hairline rounded-panel bg-ink-900/50 p-4">
      <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-1">
        <p className="min-w-0 text-body font-medium leading-snug text-ink-50">{line.label}</p>
        <p
          className={cn(
            "stat-mono shrink-0 text-body font-semibold",
      ...
      <p className="mt-1.5 max-w-prose text-detail leading-relaxed text-ink-400">{line.basis}</p>
      ...
      <Card className="border-ink-600/60">
        <CardContent className="flex items-start gap-3 p-5">
          ...
          <div className="min-w-0">
            <p className="text-detail font-medium text-ink-50">About HSA and FSA</p>
      ...
      <Card>
        <CardContent className="p-5 sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-title font-semibold text-ink-50">What a month costs</h2>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 rounded-panel border border-ink-700 bg-ink-900 p-4">
            <div className="min-w-0">
              <p className="text-detail font-medium text-ink-50">Typical month, after credit</p>
            ...
            <p className="stat-mono shrink-0 font-display text-title font-semibold text-ink-50">
          ...
          {b.oneOffs.length === 0 ? (
            ...
            <>
              ...
              <div className="mt-4 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 rounded-panel border border-ink-700 bg-ink-900 p-4">
                ...
                <p className="stat-mono shrink-0 text-heading font-semibold text-ink-50">
      ...
      <Card>
        <CardContent className="p-5 sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-title font-semibold text-ink-50">Coming up next month</h2>
          </div>
          <p className="mt-2 max-w-prose text-detail leading-relaxed text-ink-400">{b.nextMonth.basis}</p>
            ...
            <>
              <ul className="mt-4 space-y-2">
                {b.nextMonth.lines.map((l) => (
                  <li
                    key={`${l.label}-${l.on}`}
                    className="hairline flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 rounded-panel bg-ink-900/50 p-4"
                  >
                    <span className="min-w-0">
                      <span className="block text-body text-ink-50">{l.label}</span>
                      <span className="mt-1 block text-micro leading-relaxed text-ink-500">{l.basis}</span>
                    </span>
              ...
              <div className="mt-4 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 rounded-panel border border-ink-700 bg-ink-900 p-4">
                <p className="text-detail font-medium text-ink-50">Scheduled in the next 30 days</p>
                <p className="stat-mono shrink-0 text-heading font-semibold text-ink-50">
          ...
          <p className="mt-4 flex items-start gap-2 text-micro leading-relaxed text-ink-500">

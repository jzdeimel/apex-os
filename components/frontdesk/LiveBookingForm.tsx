...
export function LiveBookingForm() {
  ...
  const [confirmed, setConfirmed] = useState<{ id: string; ledgerId: string | null } | null>(null);
  ...
  useEffect(() => {
    void fetch("/api/scheduling/reference", { cache: "no-store" })
      .then(async (response) => {
        ...
        setClients(payload.clients ?? []); setStaff(payload.staff ?? []); setLocations(payload.locations ?? []);
  ...
  async function book(event: FormEvent) {
    ...
    if (!clientId || !staffId || !locationId || busy) return;
    setBusy(true); setError(null); setConfirmed(null);
    try {
      const response = await fetch("/api/appointments", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          clientId, staffId, locationId, visitType, modality,
          startAt: new Date(startAt).toISOString(), endAt: new Date(endAt).toISOString(),
          reason: reason.trim(), overrideReason: overrideReason.trim() || undefined,
          requestId: crypto.randomUUID(),
        }),
      });
      const payload = await response.json() as { appointment?: { id: string }; ledgerId?: string | null; error?: string; issues?: string[] };
      if (!response.ok || !payload.appointment) throw new Error(payload.error ?? "The appointment was not confirmed.");
      setConfirmed({ id: payload.appointment.id, ledgerId: payload.ledgerId ?? null });
    } catch (err) {
  ...
  if (confirmed) return (
    <div className="card border-optimal/30 p-6">
      ...
      <h2 className="mt-3 font-display text-title text-ink-50">Appointment confirmed</h2>
      <p className="mt-2 text-body text-ink-300">Apex appointment <span className="stat-mono">{confirmed.id}</span> and audit witness <span className="stat-mono">{confirmed.ledgerId ?? "existing idempotent request"}</span> committed together.</p>
  ...
  return (
    <form className="card space-y-5 p-5" onSubmit={book}>
      <div><p className="label-eyebrow">LIVE BOOKING</p><h2 className="mt-1 font-display text-heading text-ink-50">Create an authoritative appointment</h2><p className="mt-1 text-detail text-ink-400">Apex rechecks staff hours, Apex conflicts, patient conflicts, and connected-calendar busy time at commit.</p></div>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="text-detail text-ink-300">Staff<Select className="mt-1" value={staffId} onChange={(event) => setStaffId(event.target.value)} required>{eligibleStaff.map((person) => <option key={person.id} value={person.id}>{person.name}{person.title ? ` · ${person.title}` : ""}</option>)}</Select></label>
        <label className="text-detail text-ink-300">Visit type<Select className="mt-1" value={visitType} onChange={(event) => setVisitType(event.target.value)}><option>Coach consult</option><option>Medical visit</option><option>Follow-up</option><option>New client visit</option><option>Lab draw</option><option>Telehealth</option></Select></label>
        ...
        <label className="text-detail text-ink-300">End<Input type="datetime-local" className="mt-1" value={endAt} onChange={(event) => setEndAt(event.target.value)} required /></label>
        <label className="text-detail text-ink-300">Modality<Select className="mt-1" value={modality} onChange={(event) => setModality(event.target.value)}><option value="in-person">In person</option><option value="telehealth">Telehealth</option><option value="phone">Phone</option></Select></label>
      </div>
      ...
      <label className="block text-detail text-ink-300">Admin override reason <span className="text-ink-600">(leave blank unless a conflict is knowingly accepted)</span><Input className="mt-1" value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} /></label>
      {error && <div className="rounded-control border border-high/30 bg-high/5 p-4 text-detail text-high" role="alert">{error}</div>}
      <div className="flex justify-end"><Button type="submit" disabled={busy || !clientId || !staffId || !locationId}>{busy ? "Checking and booking…" : "Confirm appointment"}</Button></div>
    </form>
  );
}

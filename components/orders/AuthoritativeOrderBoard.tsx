...
export function AuthoritativeOrderBoard() {
  ...
  return (
    <section className="space-y-4">
      ...
      {loading && orders.length === 0 ? <p className="text-detail text-ink-500">Loading authoritative orders…</p> : orders.length === 0 ? <EmptyState icon={<PackageCheck className="h-6 w-6" />} title="No authoritative orders in scope" hint="Browser-only demonstration orders below are not fulfillment obligations." /> : (

...
export function Disclaimer({ className, compact }: { className?: string; compact?: boolean }) {
  return (
    ...
    >
      ...
      <p className={cn("text-gold-200/90", compact ? "text-[11px] leading-snug" : "text-xs leading-relaxed")}>
...
export function AiLabel({ className }: { className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-full border border-gold-400/30 bg-gold-400/10 px-2 py-0.5 text-[10px] font-medium text-gold-200",
        className,
      )}
    >

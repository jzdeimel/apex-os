...
| Opportunities and sales pipeline | Partial | Durable lead stages/events, self-claim, first-contact transition and audit witness exist; the acquisition console reads real rows. | General reassignment, configurable first-response SLA, tasks/notes, loss dialog, consult linkage, automatic transitions, bulk work queues and forecast rules. |
...
| Tasks, handoffs and service recovery | Partial/preview | Some durable tasks and clinical escalations exist; broad task/incident UI still relies on fixtures. | One authoritative work-item service, owners/queues/SLA, dependencies, comments/files, escalation, notifications, closure evidence and operational reporting. |
| Incident, complaint and quality management | Preview | Rich UI and policy concepts exist. | Durable complaint/incident record, severity, investigation, corrective action, disclosure, CAPA, attachments, retention, legal hold and regulator/insurer export. |
...
| Records, release and amendments | Missing | Audit and signed evidence foundations exist. | Designated record set, release request/authorization, redaction, export package, accounting of disclosures, amendment workflow, legal hold and continuity-of-care transfer. |

## 5. Keep patients engaged

| Capability | Status | Apex today | Required to close |
| --- | --- | --- | --- |
| Patient authentication and portal | Partial | Patient magic-link/session boundaries and authoritative messaging/community pilot exist. | Production identity decision, recovery, support, MFA/risk policy, account linking, full authoritative read model and pilot acceptance. |
...
| Support and knowledge management | Missing | No complete customer/staff support case system. | Help requests, ownership/SLA, linked patient/lead/appointment/payment context, macros/knowledge base, escalation, incident conversion and reporting. |

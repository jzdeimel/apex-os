...
| CI | **`npm run spec` — 161 checks, gating.** No test framework: Node strips types and `scripts/register-alias.mjs` supplies `@/`, so checks run against the same source the app imports. |

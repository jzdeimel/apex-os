...
| --- | --- | --- |
| Authoritative client identity and complete V1 history | The source adapter now fingerprints and accepts the real legacy production shape, maps 5,002 clients, 39 staff/legacy-author identities and 75 linked coach/medical notes, and creates no false calendar bookings. A private, non-routed queue retains 346 ambiguous items (including all 10 patientless note rows) with hashes. The aggregate inventory still includes 54,874 purchases, 1,029 communication touches, 431 routed orders, 298 shipments, documents, intake, subscriptions, inventory and audit history not yet translated. | Translate every accepted remaining domain into Apex concepts; explicitly resolve/accept 346 exception items, 14 unlinked purchases, duplicate contact identifiers and clinic ownership; reconcile source/target payloads; complete two repeatable nonprod rehearsals plus final delta; retain a documented read-only legacy fallback for anything intentionally left behind. |

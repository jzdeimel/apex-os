...
| Scheduling system of record | Availability and NCV assignment logic exist, but appointment create/reschedule/cancel is not an authoritative database workflow and live Google busy sync is not connected. | Durable staff and patient booking, reschedule, cancel/no-show, resource/room constraints, notification delivery, calendar reconciliation and rollback-safe ownership. |
| Patient-to-coach messaging | The staff handoff to Medical is now durable. The authoritative `/patient` pilot remains read-only; `/portal/messages` is a staff-only demo whose new sends live in browser state. | Patient-session-scoped message create/read APIs, coach inbox, read/delivery state, attachments policy, retention/export, urgent-language handling and live ACS/email notification transport. No direct patient-to-Medical thread. |
...
| Complete medical chart | Medical visit SOAP and NCV H&P now persist. Allergy, problem, medication/family-history tables exist but have no complete longitudinal chart editor/API; live lab and pharmacy interfaces are not connected. | Staff acceptance of allergy/problem/medication writers and reconciliation; diagnoses and adverse events; orders/results/provider review; document attachments; signed addenda; state-licensed prescribing gates. |

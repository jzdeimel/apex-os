...
| --- | --- | --- |
| Authoritative client identity and complete V1 history | The source adapter fingerprints and accepts the real legacy production shape, maps 5,002 clients, 39 staff/legacy-author identities and 75 linked coach/medical notes, and creates no false calendar bookings. It maps all 1,029 Alpha/GHL touches into immutable contact history without presenting them as native portal messages, plus 54,864 client-linked historical sales and 238,645 immutable lines with exact signed-cent reconciliation. A private, non-routed queue retains 735 review items with hashes, including all 10 patientless notes, 10 purchases without a safe patient link, 209 source item-count mismatches, 155 ownerless inbound touches and 15 attachment manifests requiring private re-housing. Routed orders, shipments, documents, intake, subscriptions, inventory and audit history are inventoried but not yet translated. | Translate every accepted remaining domain into Apex concepts; explicitly resolve/accept all 735 review items, duplicate contact identifiers and clinic ownership; re-house historical attachments; reconcile source/target payloads; complete two repeatable nonprod rehearsals plus final delta; retain a documented read-only legacy fallback for anything intentionally left behind. |

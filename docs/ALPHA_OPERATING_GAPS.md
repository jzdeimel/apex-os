...
| Complete medical chart | Medical visit SOAP, NCV H&P, append-preserving allergy/problem/outside-medication reconciliation, and the lab order→specimen→immutable result→critical alert→provider review→patient release chain now persist. Nursing cannot sign/release results. Live vendor interfaces, diagnoses, adverse events, pharmacy interfaces, attachments and signed addenda remain incomplete. | Staff acceptance of reconciliation and labs; authenticated vendor result interfaces and report files; diagnoses and adverse events; document attachments; signed addenda; state-licensed prescribing gates. |

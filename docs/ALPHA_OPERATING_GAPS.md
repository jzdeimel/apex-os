...
| --- | --- | --- |
| Authoritative client identity and complete V1 history | A SELECT-only production profile is complete and found 46 legacy `public` tables, including 5,002 profiles, 54,874 purchases, 1,029 communication touches, 431 routed orders, 298 shipment records, 81 consult-note `Appointment` rows, documents, intake and subscriptions. The original importer targets a different, newer schema and is not acceptable for rehearsal until its source adapter is replaced. | Translate the actual production schema into Apex concepts (not same-name copies); explicitly resolve 14 unlinked purchases, duplicate contact identifiers and ambiguous clinic ownership; import every accepted domain with bindings/checksums; two repeatable rehearsals; final delta; documented read-only legacy fallback for anything intentionally retained. |

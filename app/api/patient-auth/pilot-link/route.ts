...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => null)) as { clientId?: unknown; staffId?: unknown } | null;
  ...
  if (body.staffId !== undefined && (typeof body.staffId !== "string" || body.staffId.length > 128)) {
    return NextResponse.json({ ok: false, error: "staffId must be a valid staff identifier." }, { status: 400 });
  }
  try {
    const issued = await issuePatientMagicLink(body.clientId, auth.actor.id, body.staffId);
    return NextResponse.json({
      ok: true,
      signInUrl: patientSignInUrl(request.nextUrl.origin, issued.rawToken),
      expiresAt: issued.expiresAt.toISOString(),
      staffPatientLinked: Boolean(body.staffId),
